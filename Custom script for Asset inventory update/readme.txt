Check our blog for step-by-step guidance on updating asset management in IBM QRadar SIEM using custom scripts. Enhance your security setup efficiently.

Link: 


gui-asset.csv : It is a sample file that needs to be imported in IBM QRadar SIEM in first place.

asset.csv : It is a sample csv of the custom fields that need to be added or updated of related assets.

update_assets.py : Custom script for adding or updating the IBM QRadar SIEM assets.
